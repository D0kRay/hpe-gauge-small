...
public:
    ...
    void chooseSprite() override {
        ...
        if(_shootCount != 0) {
            const uint8_t next_shot = (_shootCount + 1U) % _nextShot;
            _shootCount = next_shot;
        }
    ...
    bool seePlayer() {
        ...
        for (int i = 0; i <= (int)step; ++i) {
            ...
            const int map_x = (int)floorf(x);
            const int map_y = (int)floorf(y);
            if (map_x < 0 || map_y < 0 || map_y >= FLOOR_HEIGHT || map_x >= FLOOR_WIDTH) {
                return false;
            }

            if (gameMap[map_y][map_x] > 0) {
                see = false;
                break;
            }
        }
